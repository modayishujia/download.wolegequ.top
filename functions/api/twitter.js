...
export async function onRequestGet(context) {
  ...
  try {
    const fxResp = await fetch(`https://api.fxtwitter.com/status/${tweetId}`, {
      headers: { 'User-Agent': 'Mozilla/5.0' },
    });
    if (fxResp.ok) {
      const fxData = await fxResp.json();
      if (fxData.tweet?.media) {
        const media = extractFxMedia(fxData.tweet);
        if (media.length) {
          return Response.json({
            ok: true,
            tweetId,
            author: fxData.tweet.author?.screen_name || '',
            name: fxData.tweet.author?.name || '',
            text: fxData.tweet.text || '',
            media,
          }, { headers: rateLimitHeaders(rl) });
        }
      }
      errors.push('fxtwitter: no media in response');
    } else {
      errors.push(`fxtwitter: HTTP ${fxResp.status}`);
    }
  } catch (e) {
  ...
  try {
    const synResp = await fetch(
      `https://cdn.syndication.twimg.com/tweet-result?id=${tweetId}&lang=en&token=hash`,
      {
        headers: {
    ...
    if (synResp.ok) {
      ...
      errors.push('syndication: no media');
    } else {
      errors.push(`syndication: HTTP ${synResp.status}`);
    }
  } catch (e) {
    errors.push(`syndication: ${e.message}`);
  }

  // Strategy 3: guest token GraphQL
  try {
    const guestResult = await tryGuestToken(tweetId);
    if (guestResult.ok) {
      return Response.json(guestResult, { headers: rateLimitHeaders(rl) });
    }
    errors.push('guest-token: no media');
  } catch (e) {
    errors.push(`guest-token: ${e.message}`);
  }
...
function extractFxMedia(tweet) {
  ...
  for (const v of videos) {
    media.push({
      ...
      url: v.url,
      bitrate: v.bitrate || 0,
      variants: v.variants?.map(vr => ({ url: vr.url, bitrate: vr.bitrate || 0 })) || [],
    });
  }
